...
import com.liulishuo.share.util.BitmapUtils;
...
//        mIWXAPI.sendReq(req);
    ...
    private void sendShare(final String imageUrl, final SendMessageToWX.Req req) {
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    Bitmap image = BitmapUtils.getBitmapFromUrl(imageUrl);
                    if (image != null) {
                        ...
                        req.message.thumbData = ShareUtil.bmpToByteArray(BitmapUtils.scaleCenterCrop(image, THUMB_SIZE, THUMB_SIZE));
                    }
                    // 就算图片没有了 尽量能发出分享
                    mIWXAPI.sendReq(req);
