package me.nereo.multi_image_selector.crop;
...
import me.nereo.multi_image_selector.utils.VisibleForTesting;
...
//    public static void cropImage(Activity activity, Uri source, Uri output, int maxWidth, int maxHeight){
//        new Crop(source).output(output).withMaxSize(maxWidth, maxHeight).withAspect(1,1).start(activity);
//    }

    public static void cropHearportrait(Activity activity, Uri source, Uri output, int size){
        new Crop(source).output(output).withMaxSize(size, size).withAspect(1,1).start(activity);
    }
